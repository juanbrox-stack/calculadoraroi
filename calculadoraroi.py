...
st.write("Analiza cuántas horas recuperas al mes gracias a la automatización de procesos.")

# 2. Sidebar: Parámetros generales
st.sidebar.header("Configuración")
# COMENTADO: Costes monetarios (puedes activarlos quitando el #)
...
# 5. Visualización de Resultados (Métricas de Tiempo)
st.subheader("Tiempo Recuperado")
c1, c2 = st.columns(2)
c1.metric("Horas ahorradas al mes", f"{total_horas_mes:.1f} h")
c2.metric("Horas ahorradas al año", f"{total_horas_anio:.1f} h")
...
# 8. Función para el Excel (Solo columnas de tiempo activas)
def to_excel(df):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.set_column('E:E', 20, f_decimal) # Formato para columna de horas
